...
// Prevent crash on unhandled rejections/exceptions
process.on('unhandledRejection', (reason) => {
  console.error('[FLORES] Unhandled Rejection:', reason);
});
process.on('uncaughtException', (err) => {
  console.error('[FLORES] Uncaught Exception:', err);
});
        ...
        const data = await new Promise((res, rej) => {
          https.get(wcUrl, resp => {
            if (resp.statusCode !== 200) {
              console.warn(`[FLORES] WC ${country} HTTP ${resp.statusCode}, skip`);
              resp.resume();
  ...
  try {
    ...
    const cancelledOrders = await new Promise((res, rej) => {
      https.get(cancelledUrl, resp => {
        if (resp.statusCode !== 200) {
          console.warn(`[FLORES] Cancelled fetch HTTP ${resp.statusCode}, skip`);
          resp.resume();
          rej(new Error(`HTTP ${resp.statusCode}`));
          return;
        }
...
async function initialSync() {
  ...
  for (const country of Object.keys(WC_STORES)) {
    ...
    if (!state || !state.last_sync_at) {
      ...
      console.log(`[FLORES] ${country}: no sync state, 7day fallback`);
    } else {
...
function enrichCampaignsWithProfit(campaigns, dateFrom, dateTo, orgId) {
  ...
  // Add campaigns that have WC orders but weren't in Meta API results
  // (paused/old campaigns + legacy catalog orders with non-numeric utm_campaign)
...
function getRates2(orgId) {
  ...
  const rej = {}, ship = {};
  try {
    const rows = db.prepare("SELECT key, value FROM org_settings WHERE org_id = ? AND category = 'rejection_rates2'").all(orgId);
    for (const r of rows) rej[r.key] = parseFloat(r.value);
  } catch(e) {}
  try {
    const rows = db.prepare("SELECT key, value FROM org_settings WHERE org_id = ? AND category = 'shipping_costs2'").all(orgId);
  ...
  return { rej, ship };
      ...
      if (urlPath === '/api/dashboard') {
        ...
        const chartData = db.prepare("SELECT order_date as date, COUNT(*) as orders, COALESCE(SUM(gross_eur),0) as revenue, COALESCE(SUM(profit),0) as profit FROM wc_orders WHERE order_date >= ? AND org_id = ? AND LOWER(billing_name) NOT LIKE '%test%' GROUP BY order_date ORDER BY order_date").all(d7ago, userOrgId);
        ...
        return sendJSON(res, {
          ...
          chartData: (function(){
            // Resolve active flat cut per order from tier state
            let cutPerOrder = 1;
            try {
              const tierRow = db.prepare("SELECT value FROM flores_settings WHERE key = ?").get('adv_tier_state');
              let tierState = null;
              if (tierRow && tierRow.value) { try { tierState = JSON.parse(tierRow.value); } catch(e){} }
              const _def = { light: [{max:null,cut:1}], medium: [{max:null,cut:28}], max: [{max:null,cut:35}] };
              const active = (tierState && tierState.active) || 'light';
              const ts = (tierState && tierState.presets && tierState.presets[active]) || _def[active] || _def.light;
              // With flat tiers (all cuts equal) we can just take the first
            ...
            return chartData.map(d => {
              ...
              const fb = fbMap[d.date] || { cnt: 0, prof: 0, rev: 0 };
              const fbProfitDay = (Number(fb.prof) || 0) - daySpend;          // FB profit = FB gross - ad spend
              const advProfitDay = fbProfitDay - (Number(fb.cnt) || 0) * cutPerOrder; // ADV profit = FB profit - €cut × FB orders
              return {
                date: d.date,
                orders: d.orders,
      ...
      if (urlPath === '/api/dashboard2') {
        ...
        for (const r of rows) {
          ...
          const rejRate1 = (userOrgId && userOrgId !== 1) ? ((getOrgSettings(userOrgId, "rejection_rates2")[country] != null ? parseFloat(getOrgSettings(userOrgId, "rejection_rates2")[country]) : 15) / 100) : ((dashRejectionRates[country] || 15) / 100);
